...
ColumnarTransposition::ColumnarTransposition(string key){
    this->key=key;
}

void ColumnarTransposition::setPermutationOrder() {
    for (int i = 0; i < key.length(); i++) {
        keyMap[key[i]] = i;
    }
}

string ColumnarTransposition::encrypt(const string &msg) {
    /* setPermutationOrder*/
    for (int i = 0; i < key.length(); i++) {
        keyMap[key[i]] = i;
    }
...
string ColumnarTransposition::decrypt(const string &cipher) {
